...
pub fn handle_settings_event(context: &mut Context, key_event: KeyEvent) -> EventResult {
    ...
    match key_event.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => {
            if state.focused_field + 1 < state.field_count() { state.focused_field += 1; }
        }
        ...
        KeyCode::Char(' ') | KeyCode::Enter => {
            match state.focused_field {
                0 => { context.config.general.panel_layout = context.config.general.panel_layout.next(); }
                1 => {
                ...
                2 => { context.config.general.show_preview_on_start = !context.config.general.show_preview_on_start; }
                3 => {
                    if key_event.code == KeyCode::Enter { state.editing_path = true; }
                }
                _ => {}
            }
        }
