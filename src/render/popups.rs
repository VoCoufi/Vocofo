...
pub fn popup_confirm_delete(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            Constraint::Length(2), Constraint::Length(0),
    ...
    let message = Paragraph::new(format!("Are you sure you want to delete \"{}\"?", selected_item))
        .alignment(Alignment::Center);

    let button_chunks = Layout::default()
        .direction(Direction::Horizontal)
        .constraints([
            Constraint::Percentage(10), Constraint::Percentage(35),
            Constraint::Percentage(10), Constraint::Percentage(35), Constraint::Percentage(10),
        ])
        .split(chunks[4]);
...
pub fn popup_name_creation(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    render_input_popup(frame, context, " Create folder ", "Write the name of the folder.", Color::Red)
}

pub fn popup_create_file(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    render_input_popup(frame, context, " Create file ", "Write the name of the file.", Color::Green)
}
...
pub fn popup_confirm_overwrite(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    let file_name = context.pending_paste.as_ref()
        .and_then(|(_, to, _)| {
            std::path::Path::new(to).file_name().map(|n| n.to_string_lossy().to_string())
        })
        .unwrap_or_else(|| "file".to_string());

    let area = centered_rect_dialog(frame.area(), 80, 10);
    let dialog_block = Block::default()
        .title(" Confirm Overwrite ").title_alignment(Alignment::Center)
...
pub fn popup_bookmark_list(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    ...
    let dialog_block = Block::default()
        .title(" Bookmarks ").title_alignment(Alignment::Center)
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan));
    ...
    let items: Vec<Line> = connections.iter().enumerate().map(|(i, profile)| {
        let text = format!(" {} ({}://{}@{}:{}) ", profile.name, profile.protocol, profile.username, profile.host, profile.port);
...
pub fn popup_bookmark_name(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    render_input_popup(frame, context, " Save Bookmark ", "Bookmark name:", Color::Cyan)
}
...
pub fn popup_command_palette(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    ...
    let dialog_block = Block::default()
        .title(" Command Palette ").title_alignment(Alignment::Center)
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan));
    ...
    frame.render_widget(
        Paragraph::new("─".repeat(chunks[1].width as usize)).style(Style::default().fg(Color::DarkGray)),
        chunks[1],
    );
...
pub fn popup_settings(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    ...
    let dialog_block = Block::default()
        .title(" Settings ").title_alignment(Alignment::Center)
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan));
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(1), Constraint::Length(1), Constraint::Length(1),
            Constraint::Length(1), Constraint::Length(1), Constraint::Length(1), Constraint::Min(1),
        ])
        .split(inner_area);
    ...
    let path_val = if editing_path { state.map(|s| s.path_input.as_str()).unwrap_or("") } else { &context.config.general.default_path };
    ...
    let hint = if editing_path { "Enter: confirm  Esc: cancel" } else { "↑↓: navigate  ◂▸/Space: change  Enter: edit path  Esc: save & close" };
    frame.render_widget(Paragraph::new(hint).alignment(Alignment::Center).style(Style::default().fg(Color::DarkGray)), chunks[9]);
    Ok(())
}
...
pub fn popup_connect_dialog(frame: &mut Frame, context: &mut Context) -> RenderResult<()> {
    ...
    let dialog_block = Block::default()
        .title(" Connect to Remote Server ").title_alignment(Alignment::Center)
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan));
    ...
    frame.render_widget(Paragraph::new(format!("  Protocol: {}", proto_text)).style(proto_style), chunks[0]);
    ...
    for (i, (label, value, is_password)) in fields.iter().enumerate() {
        ...
        let display_value = if *is_password && !value.is_empty() { "*".repeat(value.len()) } else { (*value).clone() };
        ...
        let para = Paragraph::new(format!("{}{}", display_value, cursor_suffix))
            ...
            .style(if is_focused { Style::default().fg(Color::White) } else { Style::default().fg(Color::Gray) });
...
fn render_input_popup(frame: &mut Frame, context: &mut Context, title: &str, label: &str, border_color: Color) -> RenderResult<()> {
    let area = centered_rect_dialog(frame.area(), 80, 10);
